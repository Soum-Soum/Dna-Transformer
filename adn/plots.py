...
def plot_trainer_logs(log_history, output_dir: Path):
    ...
    axes[1, 0].plot(acc_epochs, eval_accuracies, label="Evaluation Accuracy", color='green')
...
def plot_tsne(res, output_dir: Path, perplexity=30, n_iter=300, random_state=42):
    ...
    scatter1 = ax1.scatter(tsne_results[:, 0], tsne_results[:, 1], c=labels, cmap='viridis', alpha=0.6)
    ...
    scatter2 = ax2.scatter(tsne_results[:, 0], tsne_results[:, 1], c=pos_values, cmap='plasma', alpha=0.6)
